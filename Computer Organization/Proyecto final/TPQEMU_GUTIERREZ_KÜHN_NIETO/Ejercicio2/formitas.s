...
.equ formitas_s,0
	...
	.globl Rectangulo_degrades_violeta
...
/*
    Dibuja un rectángulo en pantalla con un degrades descendente para colores violaceos
    Parámetros:
        x0 = Posición en x
        x1 = Posición en y
		x2 = ancho del rectángulo (→)
...
Rectangulo_degrades_violeta:
    ...
    loop_Linea:
		...
		ldur x5, [sp, #8] // refresco el valor de x5
		add x1, x1, x5
		sub x4, x4, x19 //Disminuye lo que indique x6 el componente rojo
        sub x4, x4, x18 //Disminuye lo que indique x6 el componente verde
        sub x12, x12, x5 
        cbnz x12, loop_Linea //Salta el bucle si no ha llegado a la última fila       
