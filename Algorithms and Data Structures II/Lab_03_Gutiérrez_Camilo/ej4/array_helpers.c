...
void array_dump_Lab02_v1(int a[], unsigned int length) {
    fprintf(stdout, "[");
...
void array_dump_Lab02_v2(month_t a[], unsigned int length) {
    fprintf(stdout, "[");
    for (unsigned int i = 0u; i < length; ++i) {
        fprintf(stdout, "%i", (a[i]+1));
        if (i < length - 1) {
            fprintf(stdout, ", ");
        } else {
            fprintf(stdout, "]\n\n");
...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        ...
        else if (k_year > LST_YEAR)
        {
            fprintf(stderr, "Allowed years are %u (1980-2016).\n", YEARS-1);
            exit(EXIT_FAILURE);
        }
        else if (k_month > MONTHS)
        {
            fprintf(stderr, "Allowed months are %u (January-Decemeber).\n", MONTHS);
            exit(EXIT_FAILURE);
        }
        else if (k_day > LST_DAY)
        {
            fprintf(stderr, "Allowed days are %u (1st-28th).\n", DAYS);
            exit(EXIT_FAILURE);
        }     
