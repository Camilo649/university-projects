...
/*La siguiente constante representa cuantos datos esperan leerse del archivo (sin contar los strings)*/
static const int AVERAGE_DATA_MOVIE_FILE_VARS = 3;

#define EXPECTED_DATA_MOVIE_FILE_FORMAT " %u %f %f "
...
static unsigned int process_FILE(FILE *file, movie_t movielist[], unsigned int max_size) {
    ...
    while (!feof(file)) {
        ...
        readed = fscanf(file, EXPECTED_DATA_MOVIE_FILE_FORMAT, &movielist[i].runtime,
                                              &movielist[i].avg_rating,
                                              &movielist[i].n_votes);
        if (readed != AVERAGE_DATA_MOVIE_FILE_VARS)
        {
            fclose(file);
