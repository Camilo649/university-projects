...
/*Esta constante hace ealusión a la cantidad de datos esperados al leer una canción del archivo.
No tiene en cuenta el nombre de la canción ni tampoco el nombre del artista.*/
static const char EXPECTED_DAT_SONGS_FILE_VARS = 2;

#define EXPECTED_DATA_SONGS_FILE_FORMAT " %u  %u "
...
static unsigned int process_FILE(FILE *file, song_t playlist[], unsigned int max_size) {
    ...
    while (!feof(file) && i<max_size) {
        ...
        readed = fscanf(file, EXPECTED_DATA_SONGS_FILE_FORMAT, &playlist[i].year, &playlist[i].seconds);
        if (readed != EXPECTED_DAT_SONGS_FILE_VARS)
        {
            fclose(file);
