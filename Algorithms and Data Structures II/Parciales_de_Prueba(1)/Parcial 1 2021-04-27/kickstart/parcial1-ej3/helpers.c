...
static unsigned int process_FILE(FILE *file, song_t playlist[], unsigned int max_size) {
    unsigned int i=0u;
    int readed=max_size; //Agrugué esta línea porque sino no compilaba.
    readed = readed; //Agrugué esta línea porque sino no compilaba.
    while (!feof(file)) {
        ...
        fscanf(file, " %u  %u  ", &playlist[i].year, &playlist[i].seconds);
        ++i;
    }
    return (i);
}
