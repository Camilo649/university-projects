...
queue queue_disscard(queue q, unsigned int n){
    assert(invrep(q) && n < queue_size(q)); //PRE
    struct s_node * move=NULL;
    struct s_node * eraser=NULL;

    if (n == 0) //Caso para eliminar el primer elemento
    {
        queue_dequeue(q);
    }
    else //Caso general
    {
        move = q->first;
        for (unsigned int i=0u; i < n-1; ++i) //Muevo mi puntero auxiliar 'move' hasta el elemento anterior al que voy a borrar.
        {
            move = move->next;
        }
        eraser = move->next; //Apunto el elemento a borrar en mi puntero auxiliar 'eraser'.
        move->next = move->next->next; //Enlazo el elemento anterior al que voy a borrar con el siguiente del mismo.
        free(eraser);
        if (move->next == NULL) //Caso para eliminar el último elemento
        {
            q->last = move;
        }
        q->size = q->size - 1;
