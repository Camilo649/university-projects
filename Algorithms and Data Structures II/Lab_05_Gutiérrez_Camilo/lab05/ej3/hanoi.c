...
hanoi_t hanoi_init(unsigned int disk_count) {
    ...
    assert(hanoi != NULL);
    hanoi->source = stack_empty(); //Línea que agregué.
    hanoi->aux = stack_empty();
    hanoi->target = stack_empty(); //Línea que mofiqué porque el usuario de mi TAD stack está asumiendo que un stack vacío es igual a NULL.
...
hanoi_t hanoi_destroy(hanoi_t hanoi) {
    assert(hanoi != NULL);
    stack_destroy(hanoi->source); //Línea que agregué.
    stack_destroy(hanoi->aux); //Línea que agregué.
    stack_destroy(hanoi->target); //Línea que agregué.
    free(hanoi);
    return NULL;
}
