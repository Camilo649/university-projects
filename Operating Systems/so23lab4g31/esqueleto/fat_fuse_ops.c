...
#include "big_brother.h"    // Agregado
#include <alloca.h>
#include <errno.h>
#include <gmodule.h>
#include <libgen.h>
...
static void fat_fuse_read_children(fat_tree_node dir_node) {
    ...
    // Creating fs.log
    if (fat_tree_node_search(vol->file_tree, LOG_FILE) == NULL) {
        fat_file fs_log = fat_file_init(vol->table, false, LOG_FILE);
        vol->file_tree = fat_tree_insert(vol->file_tree, dir_node, fs_log);
