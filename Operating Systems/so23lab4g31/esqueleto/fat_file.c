...
static void write_dir_entry(fat_file parent, fat_dir_entry child_disk_entry,
    ...
    if (is_log_file_dentry ((unsigned char *) child_disk_entry->base_name, 
                            (unsigned char *) child_disk_entry->extension)) {
        child_disk_entry->base_name[0] = 0xe5;
        child_disk_entry->base_name[1] = 'f';
        child_disk_entry->base_name[2] = 's';
        child_disk_entry->base_name[3] = '\0';
...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        char fs_name[4] = {0xe5, 'f', 's', '\0'};
        int is_fs = strcmp((char *)disk_dentry_ptr->base_name, fs_name);

        if (is_fs == 0) {
            disk_dentry_ptr->base_name[0] = 'f';
            disk_dentry_ptr->base_name[1] = 's';
            disk_dentry_ptr->base_name[2] = '\0';
        }
...
void fat_file_unlink(fat_file file, fat_file parent) {
    ...
    u32 cluster_to_delete = current_cluster;

    //loops though all clusters occupied by the file, and marks them as free
    while (!fat_table_is_EOC(file->table, current_cluster)) {
        current_cluster = fat_table_get_next_cluster(file->table, current_cluster);
        fat_table_set_next_cluster(file->table, cluster_to_delete, FAT_CLUSTER_FREE);
        cluster_to_delete = current_cluster;
    }
...
ssize_t fat_file_pwrite(fat_file file, const void *buf, size_t size,
    ...
    if (offset > file->dentry->file_size) {
        //DEBUG ("1");      EL ERROR CON LA ESCRITURA OCURRE AQUI (borrar luego).
        errno = EOVERFLOW;
        return 0;
    }
