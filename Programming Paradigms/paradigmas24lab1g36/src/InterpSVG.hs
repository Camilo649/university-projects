...
trian1 a b c = path_ [d_ cmds]
  where
    cmds = uncurry mA a <> lR (0.0 :: Float) (0.0 :: Float) <> lR' (half b V.+ half c) <> lR' b <> lR (0.0 :: Float) (0.0 :: Float) <> z

trian2 :: FloatingPicS
trian2 a b c = path_ [d_ cmds]
  where
    cmds = uncurry mA a <> lR (0.0 :: Float) (0.0 :: Float) <> lR' b <> lR' c <> lR (0.0 :: Float) (0.0 :: Float) <> z

rect :: FloatingPicS
rect a b c = with (path_ [d_ cmds]) [fill_ "transparent", stroke_ "black"]
  where
    cmds = uncurry mA a <> lR (0.0 :: Float) (0.0 :: Float) <> lR' b <> lR' c <> lR' ((-1) V.* b) <> lR' ((-1) V.* c) <> z
